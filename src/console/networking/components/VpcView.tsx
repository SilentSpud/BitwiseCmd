    ...
    <React.Fragment>
      <div className="expression vpc-view">
        <div className="address-container">
          ...
          <div>
            ...
            <IconWithToolTip icon={faInfoCircle}>
              ...
              <span className="address-space subnet-part">000</span> - Bits dedicated for subnets address
              <br />
        ...
        <table className="vpc-details">
          <tbody>
            <tr>
              <td className="soft">VPC CIDR Mask:</td>
              <td>
                <button className="btn" onClick={decrVpc} disabled={vpc.cidr.maskBits <= 1} title="Decrease vpc address bits">
                  -
                </button>
                /{vpc.cidr.maskBits}
                <button className="btn" onClick={incrVpc} disabled={subnetMaskSize >= MAX_NON_HOSTS_BITS} title="Increse vpc address bits">
                  +
                </button>
              </td>
            </tr>
            <tr>
              <td className="soft">Subnet CIDR Mask:</td>
              <td>
                <button className="btn" onClick={decrSubnet} disabled={vpc.subnetBits <= 1} title="Increase subnet bits">
                  -
                </button>
                /{subnetMaskSize}
                <button className="btn" onClick={incrSubnet} disabled={vpc.cidr.maskBits + vpc.subnetBits >= MAX_NON_HOSTS_BITS} title="Increase subnet bits">
                  +
                </button>
              </td>
            </tr>
            <tr>
              <td className="soft">Max Subnets in VPC:</td>
              <td>
                <button className="btn" onClick={decrSubnet} disabled={vpc.subnetBits <= 1} title="Decrease subnet bits">
                  -
                </button>
                {maxSubnets}
                <button className="btn" onClick={incrSubnet} disabled={vpc.cidr.maskBits + vpc.subnetBits >= MAX_NON_HOSTS_BITS} title="Increase subnet bits">
            ...
            <tr>
              <td className="soft">Max Hosts in VPC:</td>
              <td>{maxSubnets * hostsPerSubnet}</td>
            </tr>
            <tr>
              <td className="soft">Hosts Per Subnet:</td>
