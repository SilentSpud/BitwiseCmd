...
import { IpAddress, type OctetNumber } from "../models";
...
export class IpAddressView extends React.Component<IpAddressViewProps> {
      ...
      <table className="expression">
        <tbody>
          {this.props.ipAddresses.map((ip, i) => (
            <tr key={i} className="row-with-bits">
              <td className="label">
                <strong>{ip.toString()}</strong>
              </td>
              <td className="bin">
  ...
  bin(value: number, octetNumber: OctetNumber, ip: IpAddress) {
    return <BinaryStringView binaryString={fmt(value)} key={octetNumber} emphasizeBytes={false} allowFlipBits={true} className={`octet-${octetNumber}`} onBitClicked={(e) => this.onFlippedBit(e.newBinaryString, octetNumber, ip)} />;
  }
