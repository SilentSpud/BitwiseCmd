...
export default class Operator implements ExpressionElement {
  ...
  evaluate(operand?: Operand): Operand {
    if (operand instanceof Operator) throw new Error("operand must be scalar value");

    if (this.operator != "~" && operand == null) throw new Error("operand is required");

    var evaluatedOperand = this.operand.evaluate();

    return this.operator == "~" ? applyNotOperator(this.operand.getUnderlyingOperand()) : applyOperator(operand!, this.operator, evaluatedOperand);
  }
...
function applyOperator(op1: Operand, operator: string, op2: Operand): Operand {
  ...
  if (!isShift) {
    if (op1.value.maxBitSize == op2.value.maxBitSize && op1.value.signed != op2.value.signed) throw new Error("Operator `" + operator + "` cannot be applied to signed and unsigned operands of the same " + op2.value.maxBitSize + " -bit size");
...
function equalizeSize(op1: Operand, op2: Operand) {
  ...
  if (n1.maxBitSize > n2.maxBitSize) op2.setValue(n2.convertTo(n1)).setLabel("converted");
  else op1.setValue(n1.convertTo(n2)).setLabel("converted");
}
