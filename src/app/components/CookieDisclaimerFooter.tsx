...
function CookieDisclaimerFooter(props: { appSate: AppState }): JSX.Element {
  ...
  if (props.appSate.cookieDisclaimerHidden) return <React.Fragment></React.Fragment>;

  return (
    <div className="cookie-disclaimer">
      <div className={"short" + shortCss}>
        <p>By using BitwiseCmd, you agree to the use of cookies for Google Analytics. These cookies help analyze and enhance your browsing experience. Click 'Read More' to learn about how your data is handled.</p>
      </div>
      ...
      <div>
        <p>
          <button className="button" onClick={() => setShowMore(!expanded)}>
            <FontAwesomeIcon icon={expanded ? faCaretDown : faCaretUp} />
            {buttonText}
          </button>{" "}
          <button className="button" onClick={() => props.appSate.setCookieDisclaimerHidden(true)}>
