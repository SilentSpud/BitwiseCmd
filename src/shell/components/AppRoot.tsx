...
import AppState, { type CommandResultView } from "../AppState";
import cmd from "../cmd";
...
export default class AppRoot extends React.Component<AppRootProps, AppRootState> {
  ...
  render() {
    ...
    return (
      <div className={`app-root ${this.state.uiTheme} ${newUi}`}>
        <DebugIndicators appState={this.props.appState} />
        <div className="header">
          <h1>
            Bitwise
            <span className="header-cmd">Cmd</span>
          </h1>
          <TopLinks />
        </div>
        ...
        <div className="expressionInput-container">
          ...
          <button className={settingsCss} title="Toggle Settings">
            <FontAwesomeIcon icon={faGear} onClick={() => this.props.appState.toggleShowSettings()} />
          </button>
        </div>
        {this.props.appState.showSettings ? <SettingsPane appState={this.props.appState} /> : null}
        <div id="output">{this.getResultViews()}</div>
