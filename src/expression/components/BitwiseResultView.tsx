...
import BinaryStringView, { type FlipBitEventArg as BitClickedEventArg } from "../../core/components/BinaryString";
...
export default class BitwiseResultView extends React.Component<BitwiseResultViewProps, BitwiseResultViewState> {
  ...
  render() {
    ...
    if (this.props.dimExtrBits) css += " dim-extra-bits";
  ...
  getRows(model: BitwiseResultViewModel, allowSignChange: boolean): JSX.Element[] {
    const maxAnnotatedSize = this.props.annotateTypes
      ? model.items
          .map((i) => i.maxBitSize)
          .sort()
          .reverse()[0]
    ...
    return model.items.map((itm, i) => <ExpressionElementTableRow key={i} sign={itm.sign} css={itm.css} bitSize={itm.maxBitSize} allowFlipBits={itm.allowFlipBits} allowSignChange={allowSignChange} expressionItem={itm.expressionElement} emphasizeBytes={this.props.emphasizeBytes} maxNumberOfBits={finalMaxBits} annotateTypes={this.props.annotateTypes} onValueChanged={() => this.onValueChanged()} />);
  }
...
class ExpressionElementTableRow extends React.Component<ExpressionElementRowProps> {
  ...
  render() {
    ...
    return (
      <tr className={"row-with-bits " + css}>
        <td className="sign">{sign}</td>
        <td className="label">{this.getLabel()}</td>
        <td className="bin">
          <BinaryStringView emphasizeBytes={emphasizeBytes} binaryString={bin} allowFlipBits={allowFlipBits} signBitIndex={signBitIndex} valueBitSize={valueSize} onBitClicked={(args) => this.onBitClicked(args)} />
        </td>
        <td className="other">{this.getAlternative()}</td>
        <td className="info accent1" data-test-name="ignore">
          {this.props.annotateTypes ? this.getInfo() : null}
        </td>
    ...
    if (this.scalar.value.value < 0)
      buttons.push(
        <IconWithToolTip key={key} icon={faInfoCircle}>
          ...
          {this.props.annotateTypes ? null : (
            <p>
              To see full in-memory binary representation, go to <b>Settings</b> and enable <b>Annotate Data Types</b> toggle.{" "}
            </p>
          )}
        </IconWithToolTip>,
      );
  ...
  getInfo() {
    ...
    if (allowSignChange)
      children.push(
        <button className="accent1 link-button" title={signedButtonTitle} onClick={() => this.onChangeSign()}>
          {signedStr}
        </button>,
      );
    else
