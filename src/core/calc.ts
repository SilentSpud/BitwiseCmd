import { Integer, type JsNumber, asInteger } from "./Integer";
...
export default {
  ...
  addSpace(number: Integer, requiredSpace: number): Integer {
    if (requiredSpace < 0) throw new Error("Required space cannot be negative");
  ...
  operation(op1: Integer, operator: string, op2: Integer): Integer {
    switch (operator) {
      case ">>":
        return this.rshift(op1, op2.value);
      case ">>>":
        return this.urshift(op1, op2.value);
      case "<<":
        return this.lshift(op1, op2.value);
      case "&":
        return this.and(op1, op2);
      case "|":
        return this.or(op1, op2);
      case "^":
        return this.xor(op1, op2);
      default:
  ...
  toBinaryString(num: Integer): string {
    ...
    if (bin.length > bitSize!) throw new Error(`Binary represenation '${bin}' is bigger than the given bit size ${bitSize}`);

    const r = num.value < 0 ? this.engine.applyTwosComplement(bin) : bin;
  ...
  lshift(num: Integer, numBytes: JsNumber): Integer {
    ...
    if (num.maxBitSize == numBytes) return num; // Preserve C undefined behavior
  ...
  _applyTwo(op1: Integer, op2: Integer, operation: (bin1: string, bin2: string) => string): Integer {
    if (op1.maxBitSize == op2.maxBitSize && op1.signed != op2.signed) throw new Error("This operation cannot be applied to signed and unsigned operands of the same size");
...
function checkSameLength(bin1: string, bin2: string) {
  if (bin1.length != bin2.length) throw new Error("Binary strings must have the same length");
}
...
function equalizeSize(n1: Integer, n2: Integer): [Integer, Integer] {
  ...
  return n1.maxBitSize > n2.maxBitSize ? [n1, n2.convertTo(n1)] : [n1.convertTo(n2), n2];
}
