...
export default class BinaryStringView extends React.Component<BinaryStringViewProps> {
  ...
  createBits(bitChars: string[]): JSX.Element[] {
    ...
    const firstBitIndex = this.props.valueBitSize != null ? bitChars.length - this.props.valueBitSize : -1;
    ...
    return bitChars.map((c, i) => {
      ...
      if (isExtra) className += " extra-bit";
      ...
      if (disableHighlight) className = css;

      return (
        <span className={className} title={tooltip} key={i} onClick={(e) => this.onBitClick(i, isExtra, e)}>
          {c}
        </span>
      );
    });
