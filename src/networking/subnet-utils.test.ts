...
describe("utils", () => {
  ...
  it("getNetworkAddress", () => {
    const ipm = (f: number, s: number, t: number, fr: number, m: number) => new IpAddressWithSubnetMask(new IpAddress(f, s, t, fr), m);

    expect(getNetworkAddress(ipm(192, 188, 107, 11, 12)).toString()).toBe("192.176.0.0");

    expect(getNetworkAddress(ipm(192, 168, 123, 1, 20)).toString()).toBe("192.168.112.0");

    expect(getNetworkAddress(ipm(192, 168, 123, 1, 23)).toString()).toBe("192.168.122.0");

    expect(getNetworkAddress(ipm(192, 168, 5, 125, 28)).toString()).toBe("192.168.5.112");

    expect(getNetworkAddress(ipm(255, 255, 255, 253, 30)).toString()).toBe("255.255.255.252");
  });

  it("getBroadcastAddress", () => {
    const ipm = (f: number, s: number, t: number, fr: number, m: number) => new IpAddressWithSubnetMask(new IpAddress(f, s, t, fr), m);

    expect(getBroadCastAddress(ipm(192, 188, 107, 11, 12)).toString()).toBe("192.191.255.255");

    expect(getBroadCastAddress(ipm(192, 168, 123, 1, 20)).toString()).toBe("192.168.127.255");

    expect(getBroadCastAddress(ipm(192, 168, 123, 1, 23)).toString()).toBe("192.168.123.255");

    expect(getBroadCastAddress(ipm(192, 168, 5, 125, 28)).toString()).toBe("192.168.5.127");

    expect(getBroadCastAddress(ipm(255, 255, 255, 253, 30)).toString()).toBe("255.255.255.255");
  });
